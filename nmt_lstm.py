...
def lstm_cond_layer(tparams, state_below, options, prefix='lstm', 
    ...
    showDim('state_belowc', state_belowc)
    # projected x
    state_below = tensor.dot(state_below, tparams[_p(prefix, 'W')]) + tparams[_p(prefix, 'b')]
    showDim('state_below', state_below)
    ...
    def _step(m_, x_, xc_, h_, c_, ctx_, alpha_, pctx_,
              W,U,b,Wc,Wd_att,Wi_att,U_att,c_tt,b_att): ##    
        # attention
...
#        pctx__ += xc_ ##
    ...
    if one_step:
        rval = _step(*([mask, state_below, state_belowc]+ [init_state, init_memory, None, None, pctx_]+shared_vars))
        ...
        rval, updates = theano.scan(_step, 
                                    ...
                                    outputs_info = [init_state, init_memory,
                                                    ...
                                                    None, None, None, 
                                                    None, None],
                                    non_sequences=[pctx_] + shared_vars,
                                    name=_p(prefix, '_layers'),
                                    n_steps=nsteps,
                                    profile=profile)
        return rval, updates 
...
def build_model(tparams, options):
    ...
    return trng, use_noise, x, x_mask, y, y_mask, opt_outs, cost
...
def adadelta(lr, tparams, grads, inp, cost):
    zipped_grads = [theano.shared(p.get_value() * numpy.float32(0.), name='%s_grad'%k, borrow=True) for k, p in tparams.iteritems()]
    running_up2 = [theano.shared(p.get_value() * numpy.float32(0.), name='%s_rup2'%k, borrow=True) for k, p in tparams.iteritems()]
    running_grads2 = [theano.shared(p.get_value() * numpy.float32(0.), name='%s_rgrad2'%k, borrow=True) for k, p in tparams.iteritems()]
...
#          decoder='gru_cond',
    ...
    showDim('x',x)
    showDim('x_mask',x_mask)
    showDim('y',y)
    showDim('y_mask',y_mask)
    showDim('use_noise',use_noise)
...
#            theano.printing.debugprint(f_grad_shared)
            ...
            if numpy.mod(uidx, saveFreq) == 0:
                ...
                filenameOrig000='%s.pkl' % saveto            # model.npz.pkl
                filenameDest000='%s.%s.pkl' % (saveto, str(counter000)+'.'+str(eidx))
                copyfile(filenameOrig000, filenameDest000)
                filenameOrig001=saveto                        # model.npz
                filenameDest001='%s.%s' % (saveto, str(counter000)+'.'+str(eidx))
                copyfile(filenameOrig001, filenameDest001)
                counter000+=1
                print 'Done'
